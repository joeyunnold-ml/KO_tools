...
export default function CaptureView({
  ...
  const preloaded = items.filter((i) => i.is_preloaded);
  const submitted = items.filter((i) => !i.is_preloaded);
      ...
      <div className="flex items-baseline justify-between mb-6 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-[14px] text-grey-700 mt-1">
            {items.length} item{items.length === 1 ? "" : "s"} so far · {preloaded.length} pre-loaded · {submitted.length} from participants
          </p>
        </div>
        <button
          onClick={onClose}
        ...
        >
          {busy ? "Closing…" : state.session?.blind_sort_enabled ? "🔒 Close capture · Start sorting →" : "🔒 Close capture →"}
        </button>
      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                      ...
                      {item.source ? (
                        <span className="text-[10px] text-grey-600 italic">{item.source}</span>
                      ) : null}
                    </div>
                  </div>
                  <button
                    onClick={async () => {
                      if (window.confirm(`Delete "${item.title}"?`)) await deleteItem(item.id);
                    }}
