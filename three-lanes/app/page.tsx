...
export default function LandingPage() {
  ...
  async function createSession() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/lane-sessions", { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to create session");
      saveFacilitatorToken(data.id, data.facilitator_token);
      router.push(`/lanes/${data.room_code}`);
    } catch (e) {
    ...
    <main className="flex-1 flex items-center justify-center p-6 bg-grey-100">
      <div className="w-full max-w-xl">
        <div className="mb-12 text-center">
          ...
          <p className="mt-2 text-[18px] text-grey-800">
            <span className="font-medium" style={{ color: "#0C5673" }}>🔧 Fix It</span>{" "}
            <span className="text-grey-500">·</span>{" "}
            <span className="font-medium" style={{ color: "#000F1E" }}>🧪 Test It</span>{" "}
            <span className="text-grey-500">·</span>{" "}
            <span className="font-medium" style={{ color: "#5A1430" }}>🏗️ Build It</span>
          </p>
          <p className="mt-4 text-base text-grey-800">
            Sort work into a shared decision framework. Surface where the team disagrees.
          </p>
        </div>
        ...
        {error ? <p className="mt-3 text-xs text-[var(--error-fg)] text-center">{error}</p> : null}
