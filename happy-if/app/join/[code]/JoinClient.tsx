...
import ParticipantVote from "./views/ParticipantVote";
...
export default function JoinClient({ roomCode }: { roomCode: string }) {
  ...
  // Restore participant id from sessionStorage when session loads
  useEffect(() => {
  ...
  if (!participantId) {
    ...
    if (phase === "complete") {
      return (
        <main className="flex-1 flex items-center justify-center p-6 text-center">
          <div>
            <h1 className="text-2xl font-semibold">🏁 This session is complete</h1>
            <p className="mt-2 text-grey-800">Ask the facilitator for the exported results.</p>
          </div>
        </main>
      );
    }
      ...
      <main className="flex-1 flex items-center justify-center p-6 bg-grey-100">
        ...
        >
          <div className="text-center mb-8">
            <p className="text-[12px] font-medium uppercase tracking-[2px] text-grey-700">🚪 Join room</p>
          ...
          {state.session.phase !== "lobby" && state.session.phase !== "submit" ? (
            <p className="text-xs text-grey-700 text-center">
              ℹ️ Submissions have closed, but you can still participate in voting.
            </p>
          ) : null}
        </form>
      </main>
  ...
  if (phase === "submit") return <ParticipantSubmit state={state} participantId={participantId} />;
  if (phase === "cluster") return <ParticipantCluster state={state} participantId={participantId} />;
  if (phase === "vote") return <ParticipantVote state={state} participantId={participantId} />;
