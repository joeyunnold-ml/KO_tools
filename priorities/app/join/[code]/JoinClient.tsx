...
import { joinVote } from "@/lib/actions";
import type { Team, VoteParticipantRow } from "@/lib/types";
...
export default function JoinClient({ roomCode }: { roomCode: string }) {
      ...
      <main className="flex-1 flex items-center justify-center p-6 bg-grey-100">
        <form
          ...
          onSubmit={async (e) => {
            ...
            setError(null);
            try {
              const row: VoteParticipantRow = await joinVote({
  ...
  if (phase === "vote") {
    return <ParticipantVote state={state} participantId={participantId} />;
  }
  return <ParticipantResults state={state} />;
}
