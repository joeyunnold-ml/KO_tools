...
export default function LandingPage() {
  ...
  async function createSession() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/vote-sessions", { method: "POST" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Failed to create session");
      saveFacilitatorToken(data.id, data.facilitator_token);
      router.push(`/votes/${data.room_code}`);
    } catch (e) {
      ...
      <div className="w-full max-w-xl">
        <div className="mb-12 text-center">
          ...
          <p className="mt-4 text-base text-grey-800">
            Vote on priorities, then assign owners and first actions.
            Leave the room with a commitment table.
          </p>
        </div>
